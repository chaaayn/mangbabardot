...
def StartupSmart():
    ...
    global modem
    portlist = list(serial.tools.list_ports.grep("ZTE WCDMA Technologies MSM"))
...
def SMS():
    ...
    if len(penMessage) <= 160:
        ...
        if y.startswith('OK'):
            print('Sent')
            print datetime.now()
            smart.update_one({"Status":"Pending"},{ "$set":{"Status":"Processed"}},{ "$set":{"Date":datetime.now()}})    
        elif y.startswith('+CMS') or y.startswith('^RSSI'):
            print('Failed.')
def saveMessage():
    modem.write('AT+CPMS="MT"\r\n')
    time.sleep(0.5)
    xx = modem.readlines()
    xy = str(xx)
    print xy
    if xy[30] == "1" or xy[30] == "2":    
        x = []
        ...
        sms = {"Sender":n, "Date and Time": d + " " + t, "Message":m}
        msgS.insert_one(sms)
        time.sleep(0.5)
        print "OKAY"
